...
r_tcr = row("開裂扭矩", r"T_{cr}=0.125\sqrt{f'_c}\,\frac{A_{cp}^2}{p_c}\sqrt{1+\frac{f_{pc}}{0.125\sqrt{f'_c}}}",
            r"0.79 \times 2.04\times10^{10} \times 2.72", f"{tr.Tcr:,.0f}" + BS + " kN" + BS + "cdot m",
...
{r_tcr}
<p class="note">T<sub>u</sub>=1,900 ≪ 0.25φT<sub>cr</sub>={tr.threshold:,.0f} → <b>可免顯式扭矩設計</b>，惟箱梁仍須配置閉合箍筋（D16@200）。★ 引擎以現 P_e 算 T<sub>cr</sub>={tr.Tcr:,.0f}（D2 演算 42,380 為舊 P_e）。</p>"""
...
r_f1 = row("整體區爆裂力（每束）", r"T_{burst}=0.25P_u\left(1-\frac{a}{h}\right)",
           r"0.25 \times 4{,}924 \times 0.876", f"{an.Tburst:,.0f}{kN}", "→ 爆裂筋 5-D22", True, "F1")
...
<tr><td>剝落力 F<sub>spall</sub></td><td>{an.Fspall:.0f} kN → 剝落筋 A<sub>s</sub>={an.As_spall:,.0f} mm² → 5-D22</td></tr>
